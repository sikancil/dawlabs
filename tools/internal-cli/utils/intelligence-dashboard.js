...
/**
 * Oracle Intelligence Dashboard for Oracle Intelligence Analysis Visualization
 * Provides comprehensive visualization of Oracle Intelligence sources and consensus
 */
export class OracleIntelligenceDashboard {
  constructor() {
    this.spinner = ora();
  }
  ...
  async displayIntelligenceAnalysis(packageAnalyses) {
    if (!packageAnalyses || !Array.isArray(packageAnalyses)) {
      console.log(chalk.yellow('\n⚠️ No intelligence analysis data available'));
      return;
    }
  ...
  displayOracleAgreement(pkgAnalysis) {
    ...
    const oracleIntelligenceLevel = pkgAnalysis.oracleIntelligenceLevel || 'unknown';
    const levelColors = {
    ...
    console.log(
      `   Intelligence Level: ${levelColors[oracleIntelligenceLevel]?.(oracleIntelligenceLevel.toUpperCase()) || oracleIntelligenceLevel}`,
    );
  }
  ...
  displayConflicts(conflicts) {
    ...
    conflicts.forEach((conflict, index) => {
      ...
      if (conflict.corroborated) {
        console.log(
          `      ${corroborationIcon} Corroborated by ${conflict.sourceCount} Oracle Intelligence sources`,
        );
      }
  ...
  displayOracleBreakdown(oracleResults) {
    console.log(chalk.blue('\n📡 Oracle Intelligence Sources:'));

    if (!oracleResults || oracleResults.length === 0) {
      console.log(chalk.gray('   No Oracle Intelligence sources available'));
      return;
    }
  ...
  async displayConsensusSummary(packageAnalyses) {
    if (!packageAnalyses || !Array.isArray(packageAnalyses) || packageAnalyses.length === 0) {
      console.log(chalk.yellow('\n⚠️ No package analysis data available for consensus summary'));
      return;
    }
    ...
    const highConfidencePackages = packageAnalyses.filter(p => p && p.consensusScore > 0.8).length;
    const conflictPackages = packageAnalyses.filter(
      p => p && p.analysis && p.analysis.conflicts && p.analysis.conflicts.length > 0,
    ).length;
    const readyPackages = packageAnalyses.filter(
      p => p && p.analysis && p.analysis.conflicts && p.analysis.conflicts.length === 0,
    ).length;
  ...
  async displayIntelligenceMetrics(packageAnalyses) {
    if (!packageAnalyses || !Array.isArray(packageAnalyses) || packageAnalyses.length === 0) {
      console.log(chalk.yellow('\n⚠️ No package analysis data available for metrics'));
      return;
    }

    console.log(chalk.blue.bold('\n🔍 Detailed Oracle Intelligence Metrics'));
    ...
    if (!oracleMetrics || Object.keys(oracleMetrics).length === 0) {
      console.log(chalk.gray('   No oracle metrics available'));
      return;
    }
  ...
  calculateIntelligenceEffectiveness(packageAnalyses) {
    ...
    const activeOracles = packageAnalyses.reduce(
      (sum, p) => sum + (p && p.oracleResults ? p.oracleResults.length : 0),
      0,
    );
    const avgConfidence =
      packageAnalyses.reduce((sum, p) => sum + (p && p.consensusScore ? p.consensusScore : 0), 0) /
      packageAnalyses.length;
    const consensusAgreement =
      packageAnalyses.reduce(
        (sum, p) =>
          sum + (p && p.analysis && p.analysis.oracleAgreement ? p.analysis.oracleAgreement : 0),
        0,
      ) /
      packageAnalyses.length /
      100;
  ...
  calculateOracleMetrics(packageAnalyses) {
    ...
    packageAnalyses.forEach(analysis => {
      if (
        ...
        analysis.oracleResults.length === 0
      ) {
        return;
      }
  ...
  analyzeConflicts(packageAnalyses) {
    ...
    packageAnalyses.forEach(pkgAnalysis => {
      if (
        !pkgAnalysis ||
        !pkgAnalysis.analysis ||
        !pkgAnalysis.analysis.conflicts ||
        !Array.isArray(pkgAnalysis.analysis.conflicts) ||
        pkgAnalysis.analysis.conflicts.length === 0
      ) {
        return;
      }
  ...
  displayIntelligenceRecommendations(packageAnalyses) {
    ...
    if (!packageAnalyses || !Array.isArray(packageAnalyses)) {
      console.log(chalk.yellow('\n⚠️ No intelligence analysis data available for recommendations'));
      return;
    }
    ...
    console.log(chalk.blue.bold('\n💡 Oracle Intelligence Recommendations:'));
  ...
  async displayAnalysisProgress(packageAnalyses) {
    if (!packageAnalyses || !Array.isArray(packageAnalyses) || packageAnalyses.length === 0) {
      this.spinner.warn('No packages to analyze');
      return;
    }

    this.spinner.start('Analyzing packages with Oracle Intelligence...');
    ...
    for (let i = 0; i < packageAnalyses.length; i++) {
      ...
      const oracleCount = analysis.oracleResults ? analysis.oracleResults.length : 0;
      this.spinner.text = `Analyzing ${analysis.packageName || 'Unknown package'} with ${oracleCount} oracles...`;
