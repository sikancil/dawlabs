 ...
 * Integrates our AI multi-oracle system with CI/CD workflows
 */
...
program
  ...
  .description('AI-powered deployment intelligence analysis for CI/CD workflows')
...
async function runIntelligentAnalysis() {
  const spinner = ora('🤖 Running AI intelligence analysis...').start();
  const results = [];
  ...
  try {
    ...
    spinner.text = `Analyzing ${packages.length} packages with multi-oracle intelligence...`;

    const analyzer = new MultiOraclePackageAnalyzer();
    ...
    spinner.succeed('AI intelligence analysis completed');
    ...
    spinner.fail('AI intelligence analysis failed');
...
async function main() {
  try {
    ...
    if (options.json) {
      ...
      console.log(`  🤖 Intelligence Sources: ${chalk.yellow('6/6 oracles active')}`);
    }
  } catch (error) {
