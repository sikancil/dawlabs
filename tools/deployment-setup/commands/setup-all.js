  ...
  try {
    ...
    if (!validation.valid) {
      ...
      if (!options.interactive) {
        throw new Error('Fix configuration errors before proceeding');
      }
    ...
    if (options.interactive) {
      ...
      spinner.succeed('Repository configuration skipped (use --interactive to enable)');
    }
    ...
    console.log('  node tools/deployment-setup/index.js status     - Check deployment status');
    console.log('  node tools/deployment-setup/index.js diagnose   - Troubleshoot issues');
    console.log('  node tools/deployment-setup/index.js verify     - Verify configurations\n');
  } catch (error) {
